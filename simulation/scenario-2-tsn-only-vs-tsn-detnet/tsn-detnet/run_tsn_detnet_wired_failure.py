...
print("### HYBRID TSN-DETNET WIRED FAILURE MV - FIXED MEASURED WIRED DELAY ###")
...
RESULT_DIR = os.path.join(RESULT_ROOT, "hybrid", "wired_failure", "tsn_detnet_mv")

LOG_DIR = os.path.join(RESULT_DIR, "tsn_detnet_mv_logs")
SUMMARY_FILE = os.path.join(RESULT_DIR, "tsn_detnet_mv_summary.csv")
    ...
    while step < MAX_STEPS:
        ...
        if visible and not event_triggered and all_visible_reference_distance is not None:
            ...
            if (lead_dist - all_visible_reference_distance) >= EVENT_EXTRA_TRAVEL_AFTER_ALL_VISIBLE:
                ...
                # DetNet delivery OMNeT++ 실행도 생략하고, 고정 측정 delay 기반으로 처리한다.
                print("[FIXED DETNET DELAY] skip DetNet delivery runtime simulation")

                # OMNeT++/INET을 매번 실행하지 않고, 코드에 기록된 측정 delay를 고정값으로 사용한다.
                wired_delay_sec = DEFAULT_WIRED_DELAY_SEC
                print("[FIXED WIRED DELAY] skip OMNeT++ runtime simulation")
                print(f"[FIXED WIRED DELAY] wired_delay_sec = {wired_delay_sec:.9f}s")
                ...
                for i in range(1, VEHICLE_COUNT):
                    ...
                    if node in receivers:
                        reaction_delay_sec, path_used = decide_detnet_wired_reaction(veh, wired_delay_sec)

                        if path_used == "NONE":
                            reaction_status[veh] = "wired_paths_failed"
                            print(f"[DETNET WIRED FAILURE] {veh} both wired paths failed -> no automatic braking")
                            continue

                        reaction_steps[veh] = step + max(0, int(reaction_delay_sec / STEP_LENGTH))
                        reaction_status[veh] = f"detnet_path_{path_used}"
                        print(
                            f"[DETNET WIRED RECOVERY] {veh} received via path {path_used}, "
                            f"reaction step {reaction_steps[veh]} ({reaction_delay_sec:.8f}s)"
                        )
                    else:
                        # Wired failure 조건에서 receiver 목록에 없는 것은 거리/수신 범위에 따른
                        # slow reaction으로 둔다. 실제 failure는 PATH_FAILED 또는 DetNet 경로 실패로만 처리한다.
...
def decide_detnet_wired_reaction(veh_id: str, detnet_delay_sec: float):
    ...
    if path_a_success and path_b_success:
        return FAST_REACTION_SEC + detnet_delay_sec, "A+B"
    if path_a_success:
        return FAST_REACTION_SEC + detnet_delay_sec, "A"
    if path_b_success:
        return FAST_REACTION_SEC + detnet_delay_sec, "B"

    return BOTH_PATHS_FAILED_REACTION_SEC, "NONE"
...
def main():
    ...
    print("\nDONE: tsn_detnet_mv_summary.csv and logs generated")
