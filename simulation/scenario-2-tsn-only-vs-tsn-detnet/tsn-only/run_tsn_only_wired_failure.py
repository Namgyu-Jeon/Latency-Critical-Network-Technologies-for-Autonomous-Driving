...
print("### HYBRID TSN WIRED FAILURE MV - FIXED MEASURED WIRED DELAY ###")
...
RESULT_DIR = os.path.join(RESULT_ROOT, "hybrid", "wired_failure", "tsn_mv")

LOG_DIR = os.path.join(RESULT_DIR, "tsn_mv_logs")
SUMMARY_FILE = os.path.join(RESULT_DIR, "tsn_mv_summary.csv")
...
PATH_FAILED_RECEIVERS = {"car2", "car3"}
FAILED_REACTION_SEC = 0.5
...
WIRED_DELAY_CSV = os.path.join(OMNET_DIR, "results", "tsn_delay.csv")

OMNET_INI = os.path.join(OMNET_DIR, "omnetpp_tsn.ini")
OMNET_VEC = os.path.join(OMNET_DIR, "results", "tsn_baseline.vec")
OMNET_SCA = os.path.join(OMNET_DIR, "results", "tsn_baseline.sca")
WIRED_DELAY_CSV = os.path.join(OMNET_DIR, "results", "tsn_delay.csv")
...
DEFAULT_WIRED_DELAY_SEC = 0.00003395
...
def run_single_spacing(spacing_m: float):
        ...
        if visible and not event_triggered and all_visible_reference_distance is not None:
            ...
            if (lead_dist - all_visible_reference_distance) >= EVENT_EXTRA_TRAVEL_AFTER_ALL_VISIBLE:
                ...
                for i in range(1, VEHICLE_COUNT):
                    ...
                    if veh in PATH_FAILED_RECEIVERS:
                        reaction_status[veh] = "wired_path_failed"
                        print(f"[WIRED PATH FAILURE] {veh} wired path failed -> no automatic braking")
                        continue
    ...
    summary_row = {
        ...
        "failed_receivers": "|".join(sorted(PATH_FAILED_RECEIVERS))
    }
...
def main():
    ...
    print("\nDONE: tsn_mv_summary.csv and logs generated")
